...
import '@/components/ui/dotmatrix-loader.css';
import {
    useDotMatrixPhases,
...
export function remapOpacityToTriplet(
    ...
    if (safeOpacity <= SOURCE_MID_OPACITY) {
        ...
        return Math.min(
            1,
            Math.max(0, lerpDmx(targetBase, targetMid, progress)),
        );
    }
    ...
    if (safeOpacity <= SOURCE_PEAK_OPACITY) {
        ...
        return Math.min(
            1,
            Math.max(0, lerpDmx(targetMid, targetPeak, progress)),
        );
    }
